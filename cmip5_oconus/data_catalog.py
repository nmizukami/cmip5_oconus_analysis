...
def resample_data(ds, freq='MS', region=None, chunks=None):
    ...
    for name, da in ds.data_vars.items():
        if name in ['PRCP', 'EVAP', 'total_runoff', 'RUNOFF', 'BASEFLOW']:
            out[name] = da.resample(time=freq).sum('time').where(mask)
        else:
            # TODO: weight by days in month, or sum over year
            out[name] = da.resample(time=freq).mean('time').where(mask)
...
def load_monthly_obs_meteorology(region=None, dataType=None, **kwargs):
    print('load_monthly_daymet_meteorology', flush=True)
    # for now, just load daily and resample imediately
    ds = load_daily_obs_meteorology(region=None, dataType=None, **kwargs)
